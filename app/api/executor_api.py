...
from app.api.helpers import build_updates_response, get_current_executor
...
from app.services.executor_service import (
    ...
    get_endpoints_for_executor_with_details,
    get_pods_for_executor,
)
from app.schemas.notification_payload import build_endpoint_payload, build_pod_payload
...
router = APIRouter(prefix="/executor_api", tags=["executor_api"])
...
def list_executor_endpoints(
    ...
    """Return all endpoints deployed on this executor. Full payload for reconciliation (same shape as CREATE_ENDPOINT)."""
    endpoints = get_endpoints_for_executor_with_details(db, executor.id)
    return [
        {**build_endpoint_payload(e), "status": getattr(e, "status", EndpointStatus.DEPLOYING)}
        for e in endpoints
    ]
...
@router.get("/pods")
def list_executor_pods(
    executor: Executor = Depends(get_current_executor),
    db: Session = Depends(get_db),
):
    """Return all pods assigned to this executor. Same payload shape as notification CREATE_POD/UPDATE_POD, plus status."""
    pods = get_pods_for_executor(db, executor.id)
    return [
        {**build_pod_payload(p), "status": p.status}
        for p in pods
    ]
